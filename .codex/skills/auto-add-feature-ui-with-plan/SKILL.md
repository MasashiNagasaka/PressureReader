---
name: auto-add-feature-ui-with-plan
description: 仕様策定（plan-kaizen）から実装・E2Eテスト・コミット・pushまでを一括自動実行する（E2Eテストあり版）
allowed-tools: Read, Write, Edit, Bash
caller: migrated-from-claude-command
---

# auto-add-feature-ui-with-plan skill

> Source: .claude/commands/auto-add-feature-ui-with-plan.md
> Original command: /auto-add-feature-ui-with-plan
...
上記いずれにも該当しない場合は確認なしでフェーズ2.6へ進む。

---

## フェーズ2.6: ステアリングディレクトリの事前作成（必須・省略禁止）

**目的:** フェーズ4でスキルが何らかの理由でバイパスされても、変更履歴が必ず残るようにする保険。

1. 以下のファイルを作成する（既に存在する場合はスキップ）:
   - `.steering/[YYYYMMDD]-[機能名]/requirements.md`

2. `requirements.md` の内容は仕様書（`docs/ideas/[ファイル名]`）の以下のセクションをコピーする:
   - 背景・目的
   - スコープ
   - 機能要件
   - 受け入れ条件
   - 末尾に以下を付記する:
     > この requirements.md は auto-add-feature-ui による事前生成です。Skill('add-feature-ui') 実行時に design.md と tasklist.md が追加されます。

3. フェーズ4で `Skill('add-feature-ui')` が正常に呼び出された場合、スキルが `design.md` と `tasklist.md` を追加・拡張する（`requirements.md` と重複しても問題ない）。

**フェーズ2.6が完了したら、確認や停止を一切せずにフェーズ3へ進むこと。**
...
- 修正が小さい・変更ファイルが少ないことを理由に、`Skill('add-feature-ui')` の呼び出しをスキップして直接実装すること
- スキルを経由せずにファイルを編集・作成してコミットすること
- 「短い修正なのでスキルを使わなくてよい」と判断すること

規模の大小にかかわらず、必ず `Skill('add-feature-ui')` を経由すること。
これはワークフロー遵守ルール（`.claude/rules/workflow-guardrails.md`）の要件である。
